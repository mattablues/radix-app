...
{% block body %}
    <section>
      <div class="mb-8">
        <h1 class="text-3xl font-semibold mb-2">Ny Systemuppdatering</h1>
        <p class="text-gray-600">Publicera en ny version eller milstolpe till systemets changelog.</p>
      </div>
      ...
      <form action="{{ route('admin.system-update.store') }}" method="post" class="w-full max-w-2xl border border-gray-200 p-6 rounded-xl bg-white shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="relative">
              ...
              <input type="date"
                     name="released_at"
                     id="released_at"
                     value="{{ old('released_at', date('Y-m-d')) }}"
                     class="w-full text-sm border-slate-300 rounded-md focus:border-indigo-500 focus:ring-indigo-500 transition shadow-sm">
        ...
        <div class="relative mb-8">
          ...
          <textarea name="description"
                    ...
                    class="w-full text-sm border-slate-300 rounded-md focus:border-indigo-500 focus:ring-indigo-500 transition shadow-sm">{{ old('description') }}</textarea>
        ...
        <div class="flex items-center gap-3 mb-8 ml-1">
            <input type="checkbox" name="is_major" id="is_major" value="1" {{ old('is_major') ? 'checked' : '' }}
