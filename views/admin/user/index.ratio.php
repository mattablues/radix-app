...
{% block body %}
  <section>
    ...
    <form
      ...
      action="/admin/users"
      class="mb-4"
      data-search-endpoint="/api/v1/search/users"
    >
    ...
    {% if($users['data']) : %}
                  ...
                  <div class="flex items-center justify-end gap-2">
                    ...
                    {% else : %}
                      <button
                        ...
                        data-user-id="{{ $user->getAttribute('id') }}"
                        data-user-email="{{ addslashes($user->getAttribute('email')) }}"
                      >
      ...
      <div id="users-pager" class="mt-6">
        {% if(($users['pagination']['total'] ?? 0) > ($users['pagination']['per_page'] ?? 0)) : %}
    ...
    {% else : %}
      ...
      <div id="users-pager" class="mt-6"></div>
    {% endif; %}

<!-- Modal: Blockera konto (JS-styrd) -->
    <div id="block-user-modal" class="fixed inset-0 z-[100] overflow-y-auto hidden" role="dialog" aria-modal="true" aria-hidden="true">
      <div id="block-user-backdrop" class="fixed inset-0 bg-black/60 z-0"></div>
      <div class="relative z-10 flex min-h-screen items-center justify-center p-4">
        <div class="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100">
          <h2 class="text-xl font-bold text-gray-900 mb-4">Blockera konto</h2>
          <p class="text-sm text-gray-600 mb-8">
            Detta kommer att spärra användaren <strong id="block-user-email" class="text-gray-900"></strong>.
          </p>
          <div class="flex justify-end gap-3">
            <button id="block-user-cancel" type="button" class="px-4 py-2 text-sm font-bold text-gray-500">Avbryt</button>
            <form id="block-user-form" action="" method="post">
              {{ csrf_field()|raw }}
              <button type="submit" class="bg-red-600 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-md shadow-red-100">
                Ja, blockera
              </button>
            </form>
          </div>
    ...
    <!-- Modal: Skicka aktivering (JS-styrd) -->
    <div id="activation-user-modal" class="fixed inset-0 z-[100] overflow-y-auto hidden" role="dialog" aria-modal="true" aria-hidden="true">
      <div id="activation-user-backdrop" class="fixed inset-0 bg-black/60 z-0"></div>
      <div class="relative z-10 flex min-h-screen items-center justify-center p-4">
        <div class="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100">
          <h2 class="text-xl font-bold text-gray-900 mb-4">Skicka aktivering</h2>
          <p class="text-sm text-gray-600 mb-8">
            Skicka aktiveringslänk till <strong id="activation-user-email" class="text-gray-900"></strong>?
          </p>
          <div class="flex justify-end gap-3">
            <button id="activation-user-cancel" type="button" class="px-4 py-2 text-sm font-bold text-gray-500">Avbryt</button>
            <form id="activation-user-form" action="" method="post">
              {{ csrf_field()|raw }}
              <button type="submit" class="bg-gray-900 text-white px-6 py-2 rounded-lg text-sm font-bold">
                Skicka
              </button>
            </form>
          </div>
