...
{% block body %}
    <section x-data="{ openClosedModal: null, selectedUser: { id: null, email: '' } }">
      <h1 class="text-3xl font-semibold mb-8">Stängda konton</h1>
{% if($users['data']) : %}
      <table class="w-full">
        <thead>
      ...
      >
        ...
        >
          ...
          >
            ...
            <form x-bind:action="'{{ route('admin.user.restore', ['id' => $user->getAttribute('id')]) }}?page={{ $users['pagination']['current_page'] }}'.replace('__ID__', selectedUser.id)" method="post" class="mt-3 flex justify-end space-x-2 px-2">
              {{ csrf_field()|raw }}
              <button type="submit" x-on:click="openClosedModal = false" class="relative flex items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-transparent bg-green-600 text-sm px-3 py-1 text-white hover:bg-green-700 transition-colors duration-300 cursor-pointer">
                Återställ
              </button>
              <button type="button" x-on:click="openClosedModal = false" class="relative flex items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-gray-800/20 bg-transparent text-sm px-3 py-1 text-gray-800 hover:bg-gray-800/5 transition-colors duration-300 cursor-pointer">
                Avbryt
              </button>
            </form>
          </div>
