...
{% block body %}
    <form action="{{ route('auth.login.create') }}" method="post"  class="w-full bg-white mx-3 max-w-md py-6 px-6 rounded-lg shadow-md">
      ...
      <div class="relative mb-2">
        ...
        <span class="block right-1 absolute -bottom-4 text-xs text-red-600">{{ error($errors, 'password') }}</span>
        {% endif %}
      </div>

      <div class="relative mt-6 mb-2">
        <button class="w-full whitespace-nowrap py-1.5 px-3 border border-blue-600 bg-blue-600 hover:bg-blue-700 hover:border-blue-700  transition-all duration-300 text-white rounded-lg cursor-pointer">Logga in</button>
