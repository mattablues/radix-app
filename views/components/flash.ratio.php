{% if ($session->has('flash_notification')) : %}
    {% $message = $session->flashMessage(); %}
{% endif; %}
{% $theme = $theme ?? 'public'; %}
{% $iconColor = ''; %}
{% $panelClass = ''; %}
{% $textClass = ''; %}
{% $closeClass = ''; %}
{% $badgeClass = ''; %}
{% $badgeText = ''; %}

{% if (isset($message) && is_array($message)) : %}

{% if($theme === 'app') : %}
  {% $panelClass = 'bg-white/95 border border-slate-200 shadow-xl'; %}
  {% $textClass = 'text-slate-800'; %}
  {% $closeClass = 'text-slate-300 hover:text-slate-600'; %}

  {% if($message['type'] === 'error') : %}
    {% $iconColor = 'bg-red-50 text-red-600 border border-red-100'; %}
    {% $badgeClass = 'bg-red-50 text-red-700 border border-red-100'; %}
    {% $badgeText = 'Fel'; %}
  {% elseif($message['type'] === 'success') : %}
    {% $iconColor = 'bg-emerald-50 text-emerald-600 border border-emerald-100'; %}
    {% $badgeClass = 'bg-emerald-50 text-emerald-700 border border-emerald-100'; %}
    {% $badgeText = 'Klart'; %}
  {% elseif($message['type'] === 'info') : %}
    {% $iconColor = 'bg-sky-50 text-sky-600 border border-sky-100'; %}
    {% $badgeClass = 'bg-sky-50 text-sky-700 border border-sky-100'; %}
    {% $badgeText = 'Info'; %}
  {% elseif($message['type'] === 'warning') : %}
    {% $iconColor = 'bg-amber-50 text-amber-600 border border-amber-100'; %}
    {% $badgeClass = 'bg-amber-50 text-amber-700 border border-amber-100'; %}
    {% $badgeText = 'Varning'; %}
  {% else : %}
    {% $iconColor = 'bg-slate-100 text-slate-600 border border-slate-200'; %}
    {% $badgeClass = 'bg-slate-100 text-slate-700 border border-slate-200'; %}
    {% $badgeText = 'Meddelande'; %}
  {% endif; %}
{% else : %}
  {% $panelClass = 'relative overflow-hidden bg-slate-900/95 border border-white/10 shadow-2xl'; %}
  {% $textClass = 'text-white'; %}
  {% $closeClass = 'text-white/30 hover:text-white'; %}

  {% if($message['type'] === 'error') : %}
    {% $iconColor = 'bg-red-500/15 text-red-400 border border-red-400/10'; %}
    {% $badgeClass = 'bg-red-500/15 text-red-300 border border-red-400/10'; %}
    {% $badgeText = 'Fel'; %}
  {% elseif($message['type'] === 'success') : %}
    {% $iconColor = 'bg-emerald-500/15 text-emerald-400 border border-emerald-400/10'; %}
    {% $badgeClass = 'bg-emerald-500/15 text-emerald-300 border border-emerald-400/10'; %}
    {% $badgeText = 'Klart'; %}
  {% elseif($message['type'] === 'info') : %}
    {% $iconColor = 'bg-amber-500/15 text-amber-300 border border-amber-400/10'; %}
    {% $badgeClass = 'bg-amber-500/15 text-amber-300 border border-amber-400/10'; %}
    {% $badgeText = 'Info'; %}
  {% elseif($message['type'] === 'warning') : %}
    {% $iconColor = 'bg-amber-500/15 text-amber-400 border border-amber-400/10'; %}
    {% $badgeClass = 'bg-amber-500/15 text-amber-300 border border-amber-400/10'; %}
    {% $badgeText = 'Varning'; %}
  {% else : %}
    {% $iconColor = 'bg-slate-500/15 text-slate-300 border border-white/10'; %}
    {% $badgeClass = 'bg-slate-500/15 text-slate-300 border border-white/10'; %}
    {% $badgeText = 'Meddelande'; %}
  {% endif; %}
{% endif; %}

  <div
    x-data="flashMessage"
    x-show="show"
    x-transition:enter="transition ease-out duration-300"
    x-transition:enter-start="opacity-0 -translate-y-4"
    x-transition:enter-end="opacity-100 translate-y-0"
    x-transition:leave="transition ease-in duration-200"
    x-transition:leave-start="opacity-100 translate-y-0"
    x-transition:leave-end="opacity-0 -translate-y-4"
    class="fixed top-20 left-1/2 -translate-x-1/2 z-100 w-full max-w-md px-4"
  >
    <div class="{{ $panelClass }} rounded-2xl p-4">
      {% if($theme !== 'app') : %}
        <div class="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.08),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(16,185,129,0.10),transparent_30%)]"></div>
      {% endif; %}

      <div class="relative flex items-start justify-between gap-4">
        <div class="flex items-start gap-3 min-w-0">
          <div class="p-2 rounded-xl shrink-0 {{ $iconColor }}">
            {% if ($message['type'] === 'error') : %}
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
              </svg>
            {% elseif ($message['type'] === 'success') : %}
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
              </svg>
            {% elseif ($message['type'] === 'info') : %}
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" />
              </svg>
            {% elseif ($message['type'] === 'warning') : %}
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
              </svg>
            {% else : %}
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path d="M11 3a1 1 0 10-2 0v1a1 1 0 102 0V3zM15.657 5.757a1 1 0 00-1.414-1.414l-.707.707a1 1 0 001.414 1.414l.707-.707zM18 10a1 1 0 100-2h-1a1 1 0 100 2h1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zM5 10a1 1 0 100-2H4a1 1 0 100 2h1zM8 16v-1h4v1a2 2 0 11-4 0zM12 14c.015-.34.208-.646.477-.859a4 4 0 10-4.954 0c.27.213.462.519.477.859h4z" />
              </svg>
            {% endif; %}
          </div>

          <div class="min-w-0">
            <div class="mb-1">
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest {{ $badgeClass }}">
                {{ $badgeText }}
              </span>
            </div>
            <p class="text-sm font-bold {{ $textClass }} leading-snug text-balance">
              {{ $message['body'] }}
            </p>
          </div>
        </div>

        <button @click="show = false" class="{{ $closeClass }} transition-colors cursor-pointer shrink-0 mt-0.5">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
          </svg>
        </button>
      </div>
    </div>
  </div>
{% endif; %}
