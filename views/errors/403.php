...
<head>
  ...
  <title>403 - Behörighet saknas | Radix</title>
...
<body class="bg-slate-50 text-slate-600 antialiased min-h-screen flex flex-col font-sans">
  ...
  <header class="w-full bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
    <div class="container-centered mx-auto px-6">
      <div class="h-16 flex justify-between items-center">
        ...
        <a href="<?= route('home.index') ?>" class="p-2 rounded-xl text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-all" title="Gå hem">
  ...
  <main class="grow flex items-center justify-center p-6 relative overflow-hidden">
    <!-- Bakgrundsdekor: Varningston (Amber/Slate) -->
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-[500px] bg-slate-200/30 rounded-full blur-3xl -z-10"></div>

    <div class="max-w-md w-full text-center">
      <div class="relative mb-8">
        <span class="text-[140px] font-black text-slate-200 select-none leading-none tracking-tighter opacity-50">403</span>
        <div class="absolute inset-0 flex items-center justify-center pt-8">
            <h1 class="text-2xl font-black text-slate-800 uppercase tracking-[0.2em]">Access Denied</h1>
        </div>
      </div>

      <div class="bg-white border border-gray-100 p-10 rounded-[2.5rem] shadow-2xl shadow-blue-900/5 relative z-10">
        <h2 class="text-lg font-bold text-gray-900 mb-3 uppercase tracking-wide">Behörighet saknas</h2>
        <p class="text-sm text-slate-500 mb-10 leading-relaxed">
          Ditt nuvarande användarobjekt har inte de privilegier som krävs för att exekvera denna begäran. Vänligen kontakta systemadministratören för utökade rättigheter.
        </p>

        <div class="flex flex-col gap-4">
            <a href="<?= route('home.index') ?>" class="inline-flex items-center justify-center px-8 py-4 bg-slate-900 text-white text-xs font-black uppercase tracking-widest rounded-2xl shadow-lg hover:bg-slate-800 transition-all active:scale-[0.98]">
                Återgå till säkert område
            </a>
  ...
  <footer class="py-10 border-t border-gray-100 bg-white">
    <p class="text-[10px] font-bold text-slate-400 text-center uppercase tracking-[0.3em]">
        &copy; <?= copyright((string) getenv('APP_COPY'), (string) getenv('APP_COPY_YEAR')); ?> | Radix Security
    </p>
  </footer>
