export default class SearchSystemEvents {
    constructor(options) {
    ...
    init() {
        ...
        this.pager.addEventListener('click', (e) => {
            ...
            if (!href.includes(this.routeBase)) return;
    ...
    async fetchAndRender(term, page, updateUrl) {
        ...
        try {
            ...
            // Läs text först (så vi kan logga bra fel även vid HTML/redirect)
            const rawText = await res.text();

            if (!res.ok) {
                console.error('[SystemEventsSearch] HTTP error', {
                    url: this.endpoint,
                    status: res.status,
            ...
            if (!contentType.includes('application/json')) {
                console.error('[SystemEventsSearch] Expected JSON but got:', {
                    url: this.endpoint,
                    status: res.status,
                    contentType,
                    bodyPreview: rawText.slice(0, 300)
                });
                this.renderError('Servern svarade inte med JSON (troligen redirect/HTML).');
                return;
            }
            ...
            console.error('[SystemEventsSearch] Fetch/parse error', err);
            this.renderError('Kunde inte hämta resultaten. Kontrollera console för detaljer.');
        }
    }
    ...
    renderPager(meta, term) {
        ...
        // Bygg en kompakt lista som liknar din helper (intervall=2)
        const interval = 2;
        const pages = [];
        ...
        // Desktop + mobile kan byggas mer avancerat, men detta ger samma “kärna” och CSS-klasser som dina helpers.
        this.pager.innerHTML = `
