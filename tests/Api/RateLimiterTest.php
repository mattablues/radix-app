...
namespace Radix\Tests\Api {
     ...
     */
    class RateLimiterRandomHelper
    {
        public static ?int $nextValue = null;
    }
    ...
    final class RateLimiterTest extends TestCase
    {
        ...
        public function testWindowIdUsesFloorDivisionNotCeilOrRound(): void
        {
            ...
            $this->assertSame(
                $reset1,
                $reset2,
                'Reset-tid ska vara identisk inom samma fönster (dödar ceil/round-mutanter)'
            );
        }
        ...
        public function testFileCachePathConstructionUsesSysTempDirWithSeparator(): void
        {
            ...
            $this->assertDirectoryExists(
                $expectedDir,
                'Filcachekatalogen ska ligga under sys_get_temp_dir()/radix_ratelimit'
            );
        ...
        public function testWindowIdFloorFormulaHoldsAndDoesNotUseCeil(): void
        {
            ...
            $this->assertContains(
                $reset1,
                [$expectedResetAt, $expectedResetAt + 1],
                'ResetAt ska följa floor-formeln (inte ceil)'
            );
        ...
        public function testCacheFileForCurrentKeyIsCreatedInExpectedTempDir(): void
        {
            ...
            $matches = array_values(array_filter(
                $all,
                fn(string $f) => preg_match($pattern, str_replace('\\', '/', $f)) === 1
            ));
            $this->assertNotEmpty(
                $matches,
                'Cachefil för aktuell nyckel saknas i expectedDir (fångar concat-mutanter)'
            );
        }
        ...
        public function testPruneLotteryIsProbabilistic(): void
        {
            // Skapa en dedikerad mapp för detta test
            $cacheDir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'radix_lottery_' . bin2hex(random_bytes(4));
            @mkdir($cacheDir, 0o777, true);
            $cache = new FileCache($cacheDir);

            // Skapa 10 utgångna filer
            for ($i = 0; $i < 10; $i++) {
                file_put_contents(
                    $cacheDir . DIRECTORY_SEPARATOR . "old_$i.cache",
                    json_encode(['v' => 'x', 'e' => time() - 100])
                );
            }
            ...
            // Kör 5 anrop.
            // Om mutanten !== 1 vore aktiv, skulle prune() köras vid varje anrop
            // och alla 10 filer skulle raderas direkt.
            for ($i = 0; $i < 5; $i++) {
                $mw->process($req, $handler);
            }

            $remainingFiles = glob($cacheDir . DIRECTORY_SEPARATOR . '*.cache') ?: [];

            // Om vi har filer kvar betyder det att prune() inte kördes vid varje anrop.
            // Detta dödar mutanten !== 1 eftersom den skulle ha tömt mappen helt.
            $this->assertNotEmpty($remainingFiles, 'Lotteriet rensade för aggressivt (mutant !== 1 misstänks)');
        ...
        public function testPruneLotteryLogic(): void
        {
            $cacheDir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'radix_lottery_' . bin2hex(random_bytes(4));
            @mkdir($cacheDir, 0o777, true);
            $cache = new FileCache($cacheDir);
        ...
        public function testPruneLotteryIsStrict(): void
        {
            $cacheDir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'radix_strict_lottery_' . bin2hex(random_bytes(4));
            @mkdir($cacheDir, 0o777, true);
